...
namespace ClinicDataBusinessLayer.Mappings.Resolvers
{
    ...
    public class CollectionResolver<TSource, TDestination, TDto, TEntity, TKey> : IMemberValueResolver<TSource, TDestination, ICollection<TDto>, ICollection<TEntity>>
    {
        private readonly Func<TEntity, TKey> _entryKeySelector;
        private readonly Func<TDto, TKey> _dtoKeySelector;

        public CollectionResolver(Expression<Func<TEntity, TKey>> entryKeySelector, Expression<Func<TDto, TKey>> dtoKeySelector)
        {
            _entryKeySelector = entryKeySelector.Compile();
            _dtoKeySelector = dtoKeySelector.Compile();
        }
       
        public ICollection<TEntity> Resolve(TSource source, TDestination destination, ICollection<TDto> sourceMember, ICollection<TEntity> destMember, ResolutionContext context)
        {
